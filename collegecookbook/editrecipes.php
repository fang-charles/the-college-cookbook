		...
		<form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" name="recipeForm">
			<div class="form-group">
				<div class="col-xs-6 col-xs-offset-3">
					...
					<input type="text" id="recipename" class="form-control" name="recipename" value="<?php echo $recipe['recipeName'] ?>" <?php
																																			if (!isset($_COOKIE['recipe_id'])) {
																																				echo "readonly";
																																			}
																																			?> />
			...
			<div class="form-group">
				...
				<input type="text" id="ingredient" class="form-control" name="ingredient[]" required value="<?php echo $recipe['ingredient1'] ?>" <?php
																																					if (!isset($_COOKIE['recipe_id'])) {
																																						echo "readonly";
																																					}
																																					?> />
			...
			<input type="button" class="btn btn-light" id="add" value="Add Ingredient" onclick="addIngredient()" <?php
																													if (!isset($_COOKIE['recipe_id'])) {
																														echo "disabled";
																													}
																													?> />
			...
			<div class="form-group">
				...
				<input type="text" id="steps" class="form-control" name="steps[]" required value="<?php echo $recipe['step1'] ?>" <?php
																																	if (!isset($_COOKIE['recipe_id'])) {
																																		echo "readonly";
																																	}
																																	?> />
			...
			<div id="newStep"></div>
			<input type="button" class="btn btn-light" id="add1" value="Add Step" onclick="addStep()" <?php
																										if (!isset($_COOKIE['recipe_id'])) {
																											echo "disabled";
																										}
																										?> />
